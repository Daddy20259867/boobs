...
ips.templates.set('core.tooltip', " 	<div id='{{id}}' class='ipsTooltip' role='tooltip'>{{content}}</div>");
...
ips.templates.set('core.editor.citation', " 	<div class='ipsQuote_citation ipsQuote_open'>		<a href='#' data-action='toggleQuote' aria-label='{{#lang}}toggle_quote{{/lang}}'>&nbsp;</a>		{{#contenturl}}			<a class='ipsPos_right' href='{{contenturl}}'><i class='fa fa-share'></i></a>		{{/contenturl}}		{{{citation}}}	</div>");
...
;;(function($, _, undefined) {
    ...
    ips.createModule('ips.ui.addressForm', function() {
        ...
        function minimizeAddress(elem, options) {
            const tempInput = $('<input/>').attr('type', 'text').attr('data-role', 'minimizedVersion').attr('placeholder', ips.getString('specifyLocation')).on('focus', function(e) {
                $(this).hide();
                if (options.country) {
    ...
    var editorObj = function(elem, options) {
        ...
        var reset = function() {
            ...
            if (options.minimized && options.minimizeAfterReset) {
                minimize();
                $(elem).find('.ipsComposeArea_dummy').on('focus click', function(e) {
                    $(this).off('focus click');
                    unminimize(function() {
                        focus();
                    });
    ...
    ips.createModule('ips.ui.map', function() {
        ...
        function _mapbox(elem, options) {
            ...
            function handlePopup(e) {
                ...
                ips.getAjax()(options.contentUrl + clubID).done(response => {
        ...
        return {
            ...
            afterGoogleMapsLoaded(cb) {
                ...
                if (googleMapsState === 'unloaded') {
                    ...
                    ips.loader.get(['https://maps.googleapis.com/maps/api/js?key=' + ips.getSetting('mapApiKey') + '&libraries=places&sensor=false&loading=async']).then( () => {
...
;(function($, _, undefined) {
    ...
    ips.createModule("ips.utils.serviceWorker", function() {
        ...
        const registerServiceWorker = (type, loggedIn) => {
            return navigator.serviceWorker.register(`${SERVICE_WORKER_URL}&type=${type}`).then( (registration) => {
    ...
    ips.controller.register('core.global.editor.censorBlock', {
        ...
        checkCensorBlock: function(e) {
            if (this._words.length) {
                ...
                $(display).html(XRegExp.replace($(display).html(), new RegExp("<(pre|blockquote).+?</\\1>","sig"), ''));

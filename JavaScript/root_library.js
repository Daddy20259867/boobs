    ...
    ips = (function() {
        ...
        var boot = function(config) {
            ...
            document.addEventListener('submit', e => {
                const form = e.target;
                if (!(e instanceof CustomSubmitEvent) && form instanceof HTMLFormElement && form.method.toUpperCase() === 'POST' && !getSetting('memberID') && getCsrfKeyExpiry() < Date.now() / 1000) {
                    Debug.log(`Intercepted form submission... fetching fresh CSRF key`);
                    e.preventDefault();
                    e.stopPropagation();
                    e.stopImmediatePropagation();
                    getFreshCsrfKey().then( () => {
          ...
          , getAjax = function() {
            const wrap = method => (...args) => {
                ...
                const properties = {};
                let thenResolve, thenReject, mimeTypeOverride;
                ...
                const proxyXHRObject = {
                    ...
                    set withCredentials(val) {
                        val = !!val;
                        properties.withCredentials = val;
                        if (requestInstance) {
                    ...
                    _upload: {
                        _getEventCollection(event, options) {
                            const path = [event, (options && options.capture) ? "true" : "false"];
                        ...
                        addEventListener(event, cb, options={}) {
                            ...
                            this._getEventCollection(event, options).set(cb, {
                                options,
                                event
                            });
                        },
          ...
          , _refreshCsrfKey = async function() {
            ...
            try {
                const response = await fetch(getSetting('baseURL').toString().replace(/\/+$/, '') + '/?app=core&module=system&controller=ajax&do=getCsrfKey&path=' + window.location.pathname);
                ...
                _setAjaxConfig();
                document.querySelectorAll(`form input[name="csrfKey"]`).forEach(input => {
                    input.value = data.key;
                }
                );
                document.querySelectorAll(`form:is([action*="?csrfKey="],[action*="&csrfKey="])`).forEach(form => {
                ...
                document.querySelectorAll(`[href*="?csrfKey="], [href*="&csrfKey="]`).forEach(linkLikeEl => {

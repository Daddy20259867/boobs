    ...
    ips.controller.register('core.front.core.notifications', {
        ...
        setup: function() {
            ...
            if (ips.utils.notification.needsPermission() && _.isUndefined(ips.utils.cookie.get('browserNotificationDismiss'))) {
                ...
                ips.utils.notification.getSubscription().then(subscription => {
                    if (subscription) {
                        return;
                    }
        ...
        subscribeFail: function(e, data) {
            this.scope.find('[data-action="browserNotificationPrompt"]').prop('disabled', false).text(this._buttonText);
        ...
        removeChoice: function(e) {
            ...
            choice.animationComplete(function() {
                ...
                _.each(question.find('[data-choiceID]'), function(item, idx) {
                    $(item).attr('data-choiceID', idx + 1).find('[data-role="choiceNumber"]').text(idx + 1);
                });
...
;;(function($, _, undefined) {
    ...
    ips.controller.register('core.front.core.profileCompletion', {
        ...
        dismissProfile: function(e) {
            e.preventDefault();
            var self = this;
            ips.getAjax()(ips.getSetting('baseURL') + 'index.php?app=core&module=system&controller=settings&do=dismissProfile').done(function(response) {
                self.scope.animate({
                    opacity: "0"
                }, 'fast', function() {
                    self.scope.hide();
...
;;(function($, _, undefined) {
    ...
    ips.controller.register('core.front.core.statusFeedWidget', {
        ...
        focusNewStatus: function(e) {
            ...
            ips.getAjax()(ips.getSetting('baseURL') + 'index.php?app=core&module=status&controller=ajaxcreate').done(function(response) {
        ...
        submitNewStatus: function(e) {
            ...
            ips.getAjax()(form.attr('action'), {
                ...
                self.scope.find('[data-role="statusFeed"]').prepend(newStatus).find('[data-statusID="' + response.id + '"]').hide().slideDown();
...
;;(function($, _, undefined) {
    ...
    ips.controller.register('core.front.core.tagEditor', {
        ...
        _checkMinMax: function() {
            ...
            this.scope.find('[data-action="removeTag"]').toggle(allowRemove).end().find('.ipsTags_deletable').toggleClass('ipsTags_deletable', allowRemove);
    ...
    ips.controller.register('core.front.core.dataLayer', {
        ...
        handleEvent: function(evt, event) {
            ...
            try {
                if (event._key && event._properties instanceof Object) {
                    if (!Events[event._key] || !Events[event._key].enabled)
                        return;
                    ...
                    for (let propertyKey in Properties) {
                        ...
                        if (property.enabled && !(!PII && property.pii)) {
                            ...
                            if (value !== null && value !== undefined) {
                                ...
                                if (typeof value === 'object' && Object.keys(value).length === 0 && Object.getPrototypeOf(value) === Object.prototype && types.includes('array')) {
                                    value = [];
                                } else if (!((types.includes('array') && value instanceof Array) || types.includes(typeof value))) {
        ...
        loginLogout: function() {
            ...
            if (ipsSettings.memberID) {
                ...
                ips.utils.db.set('ipsDataLayer', 'login', {
                    logged_in: 1
                }, false)
            } else {
        ...
        uniqueId: function() {
            let s = i => {
                return Math.floor((1 + Math.random()) * Math.pow(16, i)).toString(16).substring(1);
            }
            ;
            let time = Date.now();
        ...
        remoteFetchEvents: function() {
            let self = this;
            ips.getAjax()('?app=core&module=system&controller=ajax&do=getDataLayerEvents').done(function(response) {
...
;;(function($, _, undefined) {
    ...
    ips.controller.register('core.front.core.followForm', {
        ...
        _doFollowAction: function(url, data, unfollow) {
            ...
            this.scope.hide().parent('div').css({
                width: dims.outerWidth + 'px',
                height: dims.outerHeight + 'px'
            }).addClass('ipsLoading');
            ips.getAjax()(url, {
